...
export class Prefix extends Command {
    ...
    async run(message: DefiniteGuildMessage, args: string[]) {
        ...
        if (!args[0]) {
            return sendErrorEmbed(
                message.channel,
                ":x: Oops! It seems like you forgot to input a prefix to set. `."
            );
        }
        ...
        if (args[0] === this.client.guildPrefixCache.get(message.guild.id)) {
            return sendErrorEmbed(
                message.channel,
                ":x: Oops! It seems like you entered the same prefix as the one you currently use, try a different one. `."
            );
        }
