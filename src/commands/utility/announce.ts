    ...
    async run(message: DefiniteGuildMessage, args: string[]) {
        ...
        if (!annChannel) {
            return sendErrorEmbed(
                message.channel,
                ":x: Oops! It seems like you forgot to input a channel to send a message to. el> <message>`."
            );
        }
        ...
        if (!annMessage) {
            return sendErrorEmbed(
                message.channel,
                ":x: Oops! It seems like you forgot to input a message to send. el> <message>`."
            );
        }
        ...
        return (message.guild?.channels.cache.get(
            annChannel
        ) as TextChannel).send({ embeds: [annSend] });
    }
}
