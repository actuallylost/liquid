    ...
    async run(message: DefiniteGuildMessage, args: string[]) {
        ...
        if (!args[0]) {
            return sendErrorEmbed(
                message.channel,
                `:x: Oops! It seems like you forgot to enter a channel to move to.`
            );
        }
        ...
        if (!channel) {
            return sendErrorEmbed(
                message.channel,
                `:x: Oops! That channel doesn't exist, maybe you typed something wrong?`
            );
        }
        ...
        if (!(channel.type === "GUILD_VOICE")) {
            return sendErrorEmbed(
                message.channel,
                `:x: Oops! That channel isn't a voice channel, maybe you typed something wrong?`
            );
        }
    }
}
